...
for model, data, f_graph in zip(models, datas, fgs):
    ...
    try:
        os.remove("Results/5_Results_" + model + ".pkl")
    except:
        pass
    # Save results
    file = open("Results/5_Results_" + model + ".pkl", "wb")
...
try:
    os.remove("Results/5_Results.pkl")
except:
    pass
# Save results
file = open("Results/5_Results.pkl", "wb")
pickle.dump(results, file)
file.close()
