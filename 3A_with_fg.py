...
with open('Users&Keywords/checked_users.txt', 'r') as users:
    ...
    for folder in folders:
        ...
        combined_csv = pd.concat([pd.read_csv(path + line.strip().lower() + '.csv', index_col=0) for line in Lines])
...
for tense in files:
    ...
    # Import follower graphs
    follower_graph = np.loadtxt('Data/DataPrior/Follower_Graph.txt')
    fg = follower_graph.flatten()
    user_combo = []
    for user1 in users:
        for user2 in users:
            user_combo.append(user1 + '_' + user2)

    pd_fg = pd.DataFrame(np.tile(fg, (unstacked.shape[0], 1)), columns=user_combo)

    data_fg = pd.concat([unstacked, pd_fg], sort=False, axis=1)
